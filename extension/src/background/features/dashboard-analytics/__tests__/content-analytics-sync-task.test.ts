...
beforeEach(() => {
  ...
  fetchContentAnalyticsTopPosts.mockResolvedValue({
    posts: [topPost('1', Date.UTC(2026, 7, 22, 9, 0, 0), 600), topPost('2', Date.UTC(2026, 7, 21, 9, 0, 0), 400)],
    emptyState: false,
...
describe('runContentAnalyticsRangeTask', () => {
  it('always requests the post list with the Impressions metric type', async () => {
    ...
    expect(fetchContentAnalyticsTopPosts).toHaveBeenCalledWith(expect.objectContaining({ metricType: 'IMPRESSIONS' }));
  });
