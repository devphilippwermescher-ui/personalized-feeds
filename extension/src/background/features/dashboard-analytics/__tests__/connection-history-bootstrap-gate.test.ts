...
vi.mock('shared/firestore-service', () => ({
  ...
  getProfileAnalyticsConnectionHistoryJob: (...args: unknown[]) => getProfileAnalyticsConnectionHistoryJob(...args),
}));

vi.mock('../../profile-analytics/profile-analytics-history-sync', () => ({
...
describe('resolveConnectionHistoryJob', () => {
  ...
  it('scopes the job to the LinkedIn account, so a new account gets its own', async () => {
    ...
    expect(getProfileAnalyticsConnectionHistoryJob).toHaveBeenCalledWith('user-1', 'urn:li:fsd_profile:ACoAA-other');
    expect(access.created).toBe(true);
  });
