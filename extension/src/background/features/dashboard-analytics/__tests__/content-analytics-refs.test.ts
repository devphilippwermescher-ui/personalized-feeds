...
describe('Content Analytics Firestore paths', () => {
  it('scopes every document under the owning user', () => {
    ...
    expect(contentAnalyticsDailyDoc('user-1', '2026-08-22').path).toBe('users/user-1/contentAnalyticsDaily/2026-08-22');
  ...
  it('stores a preset range under its range key and a custom range under its window', () => {
    expect(contentAnalyticsRangeDoc('user-1', getContentAnalyticsRangeId('30d', '2026-07-24', '2026-08-22')).path).toBe(
      'users/user-1/contentAnalyticsRanges/30d'
    );
    expect(
