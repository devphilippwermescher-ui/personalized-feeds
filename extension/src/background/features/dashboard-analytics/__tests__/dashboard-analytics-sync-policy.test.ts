...
describe('queue deduplication', () => {
  it('drops a duplicate of the active request', () => {
    expect(selectPendingDashboardAnalyticsRequest({ trigger: 'alarm' }, null, { trigger: 'alarm' })).toBeNull();
  });
  ...
  it('keeps only the strongest follow-up trigger', () => {
    ...
    expect(
      selectPendingDashboardAnalyticsRequest({ trigger: 'alarm' }, pending, { trigger: 'linkedin_activity' })?.trigger
    ).toBe('manual');
  });
