...
import { getStorageValue, setStorageValue } from '../auth/services/authenticated-user';
...
export async function getProfileViewersSyncState(userId: string): Promise<ProfileViewersSyncState> {
  ...
  const isCurrentSchedulePolicy = state.schedulePolicyVersion === PROFILE_VIEWERS_SCHEDULE_POLICY_VERSION;
  const isCurrentSummaryCollection = state.summaryCollectionVersion === PROFILE_VIEWERS_SUMMARY_COLLECTION_VERSION;
  const isCurrentVisibleCollection = state.visibleCollectionVersion === PROFILE_VIEWERS_VISIBLE_COLLECTION_VERSION;
  const needsPrivateSummaryRescan = !isCurrentSummaryCollection && state.backfillStatus === 'complete';
  ...
  return {
    ...
    nextDueAt: isCurrentSchedulePolicy && isCurrentVisibleCollection ? migrationDueAt : now,
    retryAt: isCurrentSchedulePolicy && isCurrentVisibleCollection ? state.retryAt : undefined,
    cooldownUntil: isCurrentSchedulePolicy && isCurrentVisibleCollection ? state.cooldownUntil : undefined,
    ...
    requestBudgetTokens:
      ...
      Number.isFinite(state.requestBudgetTokens)
        ? Math.min(PROFILE_VIEWERS_BUDGET_CAPACITY, Math.max(0, state.requestBudgetTokens))
        : PROFILE_VIEWERS_BUDGET_CAPACITY,
    ...
    nextCollectionTask:
      needsPrivateSummaryRescan || !isCurrentVisibleCollection
        ? 'visible'
        : state.nextCollectionTask === 'private_summary'
          ? 'private_summary'
          : 'visible',
    privateSummaryStatus: needsPrivateSummaryRescan
      ? 'scanning'
      : state.privateSummaryStatus === 'scanning' || state.privateSummaryStatus === 'ready'
        ? state.privateSummaryStatus
        : 'not_started',
    privateSummaryNextStart: needsPrivateSummaryRescan
      ? PROFILE_VIEWERS_PAGINATION_PAGE_SIZE
    ...
    privateSummaryKnownStart:
      ...
      typeof state.privateSummaryKnownStart === 'number' &&
      state.privateSummaryKnownStart >= 0
        ? state.privateSummaryKnownStart
        : undefined,
    privateSummaryScanOrigin: needsPrivateSummaryRescan
      ? 'full'
