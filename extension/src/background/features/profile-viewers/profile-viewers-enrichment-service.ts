...
async function fetchProfileViewerPeopleSearchMatch(
  ...
  const queries = Array.from(new Set([viewer.linkedinUsername.trim(), viewer.displayName.trim()].filter(Boolean)));
...
async function enrichProfileViewerFromProfilePage(
  ...
  const peopleSearchMatch =
    ...
    !profileViewerDisplayNameConflictsWithUsername(rawPeopleSearchMatch.displayName, viewer.linkedinUsername)
      ? rawPeopleSearchMatch
      : null;
  ...
  const viewerWithTrustedData: ProfileViewerInput = {
    ...
    discardExistingProfileImage:
      viewer.discardExistingProfileImage === true || rejectedConflictingPeopleSearchIdentity || undefined,
  };
  const createResult = (
  ...
  try {
    ...
    if (
      ...
      (metadata.linkedinUsername && metadata.linkedinUsername.toLowerCase() !== viewer.linkedinUsername.toLowerCase())
    ) {
...
export async function enrichVisibleProfileViewers(
  ...
  const enrichments = await mapWithConcurrency(viewers, PROFILE_VIEWERS_ENRICHMENT_CONCURRENCY, async (viewer) => {
    ...
    const storedIdentityConflicts = Boolean(
      storedViewer && profileViewerDisplayNameConflictsWithUsername(storedViewer.displayName, viewer.linkedinUsername)
    );
    ...
    const viewerWithIdentitySafety =
      storedIdentityConflicts || parsedIdentityConflicts || ignoredDuplicateExistingImage
        ? {
            ...viewer,
            identityUncertain: parsedIdentityConflicts || viewer.identityUncertain === true,
    ...
    const existingViewer = storedViewer
      ? {
          ...storedViewer,
          profileImageUrl: ignoredDuplicateExistingImage || storedIdentityConflicts ? '' : storedViewer.profileImageUrl,
        }
      : undefined;

    if (
      !options.verifyEveryIdentity &&
      !profileViewerNeedsEnrichment(viewerWithIdentitySafety, storedViewer, ignoredDuplicateExistingImage)
    ) {
...
export function updateExistingProfileViewerSnapshot(
  ...
  viewers.forEach((viewer, index) => {
    ...
    const mergedViewer = mergeProfileViewerWithPageMetadata(viewer, { displayName: '', profileImageUrl: '' }, existing);
