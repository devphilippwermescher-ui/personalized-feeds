...
export async function syncProfileViewersViaApi(
  ...
  while (true) {
    ...
    rankedViewerCandidates.splice(0, rankedViewerCandidates.length, ...collectionWindow.rankedViewers);
    ...
    const reachedVisibleLimit =
      typeof visibleViewerLimit === 'number' && rankedViewerCandidates.length >= visibleViewerLimit;
    ...
    if (!canMakeProfileViewersRequest(syncState, Date.now(), options.requestBudgetReserve || 0)) {
      break;
    }
