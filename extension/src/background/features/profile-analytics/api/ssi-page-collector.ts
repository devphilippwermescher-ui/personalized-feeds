...
export async function collectSocialSellingIndexInLinkedInPage(
  ...
  function collectVisibleMemberScore(): number | undefined {
    ...
    const value =
      text.match(/current\s+social\s+selling\s+index[\s\S]{0,500}?(\d{1,3})(?:\s+out\s+of\s+100|\s*\/\s*100)/i)?.[1] ||
      text.match(/\b(\d{1,3})(?:\s+out\s+of\s+100|\s*\/\s*100)\b/i)?.[1];
  ...
  function visibleScoreResponse(status?: number): LinkedInSsiPageResponse | null {
    ...
    return typeof score === 'number' ? { ok: true, status, payload: { memberScore: { overall: score } } } : null;
  }
  ...
  try {
    ...
    const overall =
      memberScore && typeof memberScore === 'object' ? (memberScore as { overall?: unknown }).overall : undefined;
