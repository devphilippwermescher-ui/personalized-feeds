...
import type { LinkedInSsiPageResponse } from './api/ssi-page-collector';
...
async function fetchFromLinkedInTab(linkedInTabId: number, csrfToken: string): Promise<LinkedInSsiPageResponse | null> {
  try {
...
export async function fetchSocialSellingIndexSnapshot(
  ...
  const failedResponse: LinkedInSsiPageResponse = activePageResponse ||
    backgroundResponse || {
      ok: false,
