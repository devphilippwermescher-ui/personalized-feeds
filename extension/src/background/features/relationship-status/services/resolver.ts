...
function canBeOverriddenByUnavailableStatus(resolution: RelationshipResolution): boolean {
...
export async function resolveLinkedInRelationshipStatusInBackground(
  ...
  for (const queryId of GRAPHQL_QUERY_IDS) {
    try {
      const graphQLResult = await fetchGraphQLRelationshipStatusInBackground(normalizedUsername, queryId, csrfToken);
      if (graphQLResult) {
...
export async function sendLinkedInConnectRequestInBackground(profileUrn: string, referrerUrl?: string): Promise<void> {
  ...
  const response = await fetchWithTimeout(LINKEDIN_CONNECT_REQUEST_URL, init, LINKEDIN_CONNECT_REQUEST_TIMEOUT_MS);
