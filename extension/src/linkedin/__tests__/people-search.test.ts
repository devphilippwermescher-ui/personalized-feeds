...
function person(username: string, displayName: string, imageId: string): Record<string, unknown> {
  return {
            ...
            nonEntityProfilePicture: {
              vectorImage: {
                ...
                artifacts: [
                  {
                    width: 200,
                    fileIdentifyingUrlPathSegment: 'profile-displayphoto-shrink_200_200/photo?e=4102444800',
                  },
                ],
              },
            },
...
describe('LinkedIn people search identity matching', () => {
  it('returns the avatar from the exact username result, not the first profile', () => {
    ...
    const result = findLinkedInPeopleSearchResultByUsername(payload, 'yuliia-biliavtseva');
