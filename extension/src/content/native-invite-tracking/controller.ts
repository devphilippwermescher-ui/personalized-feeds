...
function isInviteSubmitElement(element: Element): boolean {
  ...
  return (
    /invitation/i.test(dialogText) && /^(?:send(?:\s+without\s+(?:a\s+)?note)?|send invitation)$/i.test(actionText)
  );
}
...
function findProfileLink(root: Element, actionElement: Element): HTMLAnchorElement | null {
  ...
  const links = Array.from(root.querySelectorAll<HTMLAnchorElement>('a[href*="/in/"]')).filter((link) =>
    getUsernameFromLinkedInUrl(link.href)
  );
...
function findProfileRoot(username: string): Element | null {
  ...
  const profileLink = Array.from(document.querySelectorAll<HTMLAnchorElement>('a[href*="/in/"]')).find(
    (link) => getUsernameFromLinkedInUrl(link.href) === normalizedUsername
  );
...
function isPendingInviteVisible(context: ConnectClickContext): boolean {
  ...
  const root =
    currentUsername === context.linkedinUsername
