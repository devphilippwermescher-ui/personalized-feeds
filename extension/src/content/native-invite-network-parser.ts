...
export function isInviteCreationRequest(rawUrl: string, requestBody = ''): boolean {
  ...
  return isKnownVoyagerCreateEndpoint || isKnownMyNetworkRscEndpoint ||
...
export function isSuccessfulInviteCreationResponse(rawUrl: string, responseText: string): boolean {
  ...
  if (
    ...
    return /"stringValue"\s*:\s*"Pending"/i.test(decodedResponse) &&
      !/"errors"\s*:\s*\[(?!\s*\])/i.test(decodedResponse);
  }
...
export function parseNativeInviteNetworkResult(
  ...
  const memberNumericId = text.match(/urn:li:member:(\d+)/)?.[1] ||
    extractField(text, ['memberId', 'memberNumericId']);
